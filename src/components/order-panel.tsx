...
function OrderPanelContent({ projectId, className }: OrderPanelProps) {
  ...
  const startStripeCheckout = async () => {
    ...
    try {
      ...
      const cadRaw = dualStorageGet(`node0-cad-shell:${projectId}`);
      let cadDocument: unknown = null;
      if (cadRaw) {
        try {
          cadDocument = parseCadDocumentUnknown(JSON.parse(cadRaw)) ?? null;
        } catch {
          cadDocument = null;
        }
      }
      const circuitron = loadCircuitronForProject(projectId);
      let pcbSnapshot: unknown = null;
      if (circuitron && typeof circuitron === "object") {
        const cr = circuitron as Record<string, unknown>;
        const w =
          typeof cr.widthMm === "number"
            ? cr.widthMm
            : typeof cr.pcbWidthMm === "number"
              ? cr.pcbWidthMm
              : null;
        const h =
          typeof cr.heightMm === "number"
            ? cr.heightMm
            : typeof cr.pcbHeightMm === "number"
              ? cr.pcbHeightMm
              : null;
        if (w != null && h != null) pcbSnapshot = { widthMm: w, heightMm: h };
      }
  ...
  return (
    <div
      className={cn(
        "flex min-h-0 flex-1 flex-col bg-[#0b0a08]",
        className,
      )}
    >
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-xs uppercase tracking-[0.16em] text-zinc-500">
                          {safeSlug} · {o.qty} pcs
                          {o.source === "stripe" ? (
                            <span className="ml-2 rounded border border-[#635bff]/40 bg-[#635bff]/15 px-1.5 py-0.5 text-[10px] font-medium normal-case tracking-normal text-teal-200">
                              Stripe
                            </span>
                          ) : null}
                          {paidLabel ? (
  ...
  return (
    <Suspense
      fallback={
        <div
          className={cn(
            "flex min-h-0 flex-1 items-center justify-center bg-[#0b0a08] text-sm text-zinc-500",
            props.className,
          )}
        >
          Loading orders…
