...
function ArInner() {
      ...
      <main className="fixed inset-0 z-[60] min-h-dvh bg-black text-zinc-100">
        ...
        <div className="pointer-events-none absolute inset-x-0 top-0 z-20 flex flex-col gap-2 pt-[max(0.5rem,env(safe-area-inset-top))]">
          <div className="pointer-events-auto flex items-center justify-between gap-2 px-3">
            ...
            {viewArjs ? (
              <button
                ...
                className="rounded-full border border-amber-500/40 bg-amber-500/15 px-2.5 py-1.5 text-[10px] font-medium text-amber-200 backdrop-blur-md active:bg-amber-500/25"
              >
                Marker
              </button>
            ) : (
          ...
          <div className="pointer-events-auto mx-3 flex gap-2 rounded-2xl border border-white/12 bg-black/55 p-3 backdrop-blur-xl">
            <Info className="mt-0.5 size-4 shrink-0 text-amber-300/90" aria-hidden />
        ...
        {steps.length > 0 ? (
          <div className="pointer-events-none absolute inset-x-0 bottom-0 z-[40] max-h-[52vh] pb-[max(1rem,env(safe-area-inset-bottom))] px-3">
            <div className="pointer-events-auto overflow-hidden rounded-[1.5rem] border border-white/[0.08] bg-[#0b0a08]/80 shadow-[0_-8px_50px_rgba(0,0,0,0.7)] backdrop-blur-3xl ring-1 ring-white/[0.02]">
              <div className="border-b border-white/[0.06] bg-white/[0.02] px-4 py-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span
                    className="text-[11px] font-medium tabular-nums text-amber-400"
                    key={activeStep}
                  >
                  ...
                  {steps.map((_, i) => (
                    <span
                      ...
                      className={cn(
                        ...
                        i === activeStep ? "bg-amber-400" : "bg-zinc-700",
                      )}
                      aria-hidden
                    />
                  ))}
              ...
              <div className="max-h-[min(32vh,280px)] overflow-y-auto px-5 py-4">
                ...
                {highlightRef ? (
                  <div className="mt-3 inline-flex items-center rounded-lg border border-amber-500/20 bg-amber-500/10 px-2.5 py-1 font-mono text-xs font-semibold text-amber-400">
                    {highlightRef}
                  </div>
                ) : null}
                {pcbNotice ? (
  ...
  return (
    <main className="min-h-screen bg-[#0b0a08] text-zinc-100">
                  ...
                  <div className="relative">
                    ...
                    <div className="mt-5 flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={onEnter}
                        className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-500 px-4 py-2 text-sm font-semibold text-amber-950 shadow-[0_14px_40px_rgba(14,165,233,0.25)] transition hover:bg-amber-400"
                      >
                        Enter AR preview
                      </button>
                      {qrSrc ? (
